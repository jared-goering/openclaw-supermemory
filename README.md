...
Tested against [LongMemEval](https://xiaowu0162.github.io/long-mem-eval/), the standard benchmark for long-term conversational memory (500 questions across 6 categories):

| Category | Accuracy | Notes |
|----------|----------|-------|
| Knowledge updates | 100% | Tracks fact changes correctly |
| Assistant fact recall | 100% | Remembers what it said |
| Preference tracking | 100% | User preferences retained |
| User fact retrieval | 50% | Single missed certification event |
| Multi-session reasoning | 0% | Requires cross-session aggregation |
| Temporal reasoning | 37.5% | Date math + event retrieval gaps |
| **Overall** | **55%** | 20-question sample with container isolation |

32ms median search latency. Entirely local on SQLite. No API keys required for search.

**Where it's strong:** Direct fact recall, preference tracking, knowledge updates. These are the bread-and-butter production use cases.

**Where it's weak:** Counting across sessions ("how many weddings this year?") and temporal math ("how many days between X and Y?"). These require structured event retrieval, not just embedding similarity. See our [detailed failure analysis](docs/BENCHMARK_ANALYSIS.md) and the [v0.3 roadmap](#roadmap) for how we're fixing this.

### Roadmap

The [benchmark analysis](docs/BENCHMARK_ANALYSIS.md) (generated by GPT 5.4) identified 5 architectural improvements ranked by impact:

1. **Structured event layer** — Extract typed event records (date, duration, status) alongside free-text memories. Turn counting queries into structured lookups.
2. **Hybrid retrieval + query routing** — Classify queries and route to dense, sparse, or structured search. Stop returning plans when the question asks about completed events.
3. **Canonical event clustering** — Deduplicate "attended Jen's wedding" / "friend's wedding last weekend" into one canonical event.
4. **Deterministic temporal reasoning** — Parse "10 days ago" programmatically instead of letting the LLM guess at date math.
5. **Recall expansion + diagnostics** — Multi-query retrieval with answerability scoring.

Target: v0.3 with fixes 1-2, v0.4 with fixes 3-5.
